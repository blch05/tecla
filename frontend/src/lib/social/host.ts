...
export function startRound(prev: GS, now: number, rand: () => number): GS {
  ...
  if (gs.game === 'oculta') {
    ...
    const giver = gs.roster[(gs.round - 1) % gs.roster.length].id;
...
export function act(prev: GS, from: string, a: Act, now: number): Result {
  ...
  if (gs.game === 'oculta' && gs.o && gs.phase === 'playing' && a.type === 'guess') {
    ...
    return { gs: maybeEnd(gs, now), changed: true, reply: { to: from, event: 'marks', payload: { g: norm(String(a.g)), marks } } };
  }
  ...
  if (gs.game === 'sopa' && gs.s && gs.phase === 'playing' && a.type === 'pick') {
    ...
    return { gs: maybeEnd(gs, now), changed: true };
  }
  ...
  if (gs.game === 'tutti' && gs.t) {
    ...
    if (a.type === 'ready' && gs.phase === 'vote' && !t.ready.includes(from)) {
      t.ready.push(from);
      if (gs.roster.every(r => t.ready.includes(r.id))) { gs.until = now; return { gs: tick(gs, now, Math.random).gs, changed: true }; }
      return { gs, changed: true };
    }
  }
  return none;
...
function maybeEnd(gs: GS, now: number): GS {
  if (gs.game === 'oculta' && gs.o) {
    const done = gs.roster.every(r => gs.o!.solved[r.id] != null || (gs.o!.rows[r.id]?.length || 0) >= MAX_TRIES);
...
export function tick(prev: GS, now: number, rand: () => number): { gs: GS; changed: boolean } {
  if (now < prev.until || prev.phase === 'over') return { gs: prev, changed: false };
  let gs = clone(prev);
  if (gs.phase === 'playing') {
    if (gs.game === 'tutti' && gs.t) {
    ...
    gs = gs.round >= gs.rounds ? { ...gs, phase: 'over', until: Infinity } : startRound(gs, now, rand);
  }
  return { gs, changed: true };
}
